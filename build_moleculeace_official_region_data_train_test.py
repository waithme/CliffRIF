...
4. Region atoms are localized first with the single-cut MMP procedure defined
   ...
   ``cliff_mol=1``.
5. Train and test outputs are generated separately. Train molecules use only
   TRAIN references, while test molecules use only TEST references. Therefore
   neither split can leak activity values or region references from the other.
6. Both JSONL files and the summary are written under ``train_test_region/``
   by default. Train and test rows use the same output schema.

Example
-------
python build_moleculeace_official_region_data_train_test.py \
  --input_dir data/moleculeace \
  --output_dir train_test_region
"""
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Build split-isolated train and test MoleculeACE region supervision"
    )
    parser.add_argument("--input_dir", required=True, help="Directory containing the 30 MoleculeACE CSV files")
    parser.add_argument("--output_dir", default="train_test_region",
                        help="Directory containing train, test, and summary outputs")
    parser.add_argument("--output", default=None,
                        help="Optional train JSONL filename (parent directory is ignored)")
    parser.add_argument("--test_output", default=None,
                        help="Optional test JSONL filename (parent directory is ignored)")
...
def main():
    ...
    output = output_dir / (Path(args.output).name if args.output else
                           "moleculeace_official_train_regions_v2.jsonl")
    test_output = output_dir / (Path(args.test_output).name if args.test_output else
                                "moleculeace_official_test_regions_v2.jsonl")
    if output == test_output:
        raise ValueError("Train and test output filenames must be different")
    train_items, test_items, per_dataset = [], [], {}

    for dataset_index, csv_path in enumerate(csv_files, 1):
        rows, invalid = read_dataset(csv_path, args)
        fragment_rows(rows, args)
        train = [row for row in rows if row["split"] == "train"]
        test = [row for row in rows if row["split"] == "test"]

        # Train anchors are deliberately restricted to train references.
        train_effects, train_stats = collect_effects(train, lambda _: train, args)
        # Test anchors are strictly restricted to test references. They never
        # inspect train labels, train molecules, or train-derived regions.
        test_effects, test_stats = collect_effects(test, lambda _: test, args)

        train_items.extend((row, train_effects.get(row["row_idx"], {})) for row in train)
        test_items.extend((row, test_effects.get(row["row_idx"], {})) for row in test)
        per_dataset[csv_path.stem] = {
            "valid_rows": len(rows), "invalid_rows": invalid,
            "train_rows": len(train), "test_rows": len(test),
            "train_region_search": train_stats,
            "test_region_search": test_stats,
        }
    ...
    test_counts = write_items(test_output, test_items)
    summary = {
        "input_dir": str(input_dir), "datasets": len(csv_files),
        "train_output": str(output), "test_output": str(test_output),
        "official_definition": {
        ...
        "leakage_policy": "train uses train references only; test uses test references only",
        "region_policy": f"{args.localization_methods}, official cliff anchors only",
        "train": train_counts, "test": test_counts,
        "per_dataset": per_dataset,
    }
    summary_path = output_dir / "moleculeace_official_train_test_regions_v2_summary.json"
